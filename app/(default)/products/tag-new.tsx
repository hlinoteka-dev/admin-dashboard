...
import { Tag } from '@/components/dropdown-tags'
import axios from 'axios'

export default function TagNew({ tag }: { tag: Tag }) {
	...
	return (
		<>
			{/* Start */}
			<button className="text-rose-500 hover:text-rose-600 rounded-full" onClick={() => { setDangerModalOpen(true) }} type="button">
			...
			<ModalBlank isOpen={dangerModalOpen} setIsOpen={setDangerModalOpen}>
				<div className="p-5 flex space-x-4">
					...
					<div>
						{/* Modal header */}
						<div className="mb-2">
							<div className="text-lg font-semibold text-slate-800 dark:text-slate-100">Delete {tag.name}?</div>
						</div>
						{/* Modal content */}
						<div className="text-sm mb-10">
							<div className="space-y-2">
								<p>This action cannot be undone. This will permanently delete <span className="font-bold">{tag.name}</span>.</p>
							</div>
						</div>
						{/* Modal footer */}
